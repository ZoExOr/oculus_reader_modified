# Modified by Yijun Zhou in 2025. Original code from https://github.com/rail-berkeley/oculus_reader
...
class OculusReader:
    ...
    def restart_app(self):
        try:
            self.device.shell('am force-stop com.rail.oculus.teleop')
            time.sleep(1.0)
            self.device.shell('am start -n com.rail.oculus.teleop/.MainActivity')
            return True
        except Exception as e:
            eprint("Failed to restart APK:", e)
            return False
    ...
    def process_data(string):
        ...
        # timestamp = time.time()
        # for side, mat in transforms.items():
        #     csv_writer.writerow(
        #         [timestamp, side, mat, buttons]
        #     )
    ...
    def get_transformations_and_buttons(self):
        # block until we have right-controller ('r') transform
        while self.running:
            with self._lock:
                transforms = self.last_transforms
                buttons = self.last_buttons
            if transforms and ('r' in transforms) and (transforms['r'] is not None):
                return transforms, buttons
            time.sleep(0.01)
    ...
    def wait_until_ready(self, timeout_s=5.0):
        """
        wait till get valid transforms.
        Auto restart if timeout.
        """
        backoff = 1.0
        while self.running:
            t0 = time.time()
            while self.running and (time.time() - t0) < timeout_s:
                with self._lock:
                    ok = (
                        self.last_transforms
                        and ('r' in self.last_transforms)
                        and (self.last_transforms['r'] is not None)
                    )
                if ok:
                    return True
                time.sleep(0.05)

            # if time out, auto restart apk
            eprint(f"Reader not ready within {timeout_s}s, please put on headset, we will restart APK for you...")
            self.restart_app()
            time.sleep(backoff)
